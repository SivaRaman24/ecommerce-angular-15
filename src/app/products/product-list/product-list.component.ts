...
import { Product, ProductListParams } from '../../shared/model/product.interface';
...
export class ProductListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.queryParams$ = this.activatedRoute.queryParams.subscribe(
      (queryParams) => {
        if(Object.keys(queryParams).length && queryParams.hasOwnProperty('c')) {
          this.queryParams['c'] = queryParams['c'];
          this.selectedCatgory = queryParams['c'];
        }

        if(Object.keys(queryParams).length && queryParams.hasOwnProperty('s')) {
  ...
  onSortByChangeEventHandler(sortBy: any) {
    this.sortBy = sortBy;
    if(!sortBy) {
