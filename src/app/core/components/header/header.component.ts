...
import { Product } from 'src/app/shared/model/product.interface';
...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.totalCartItemSub$ = this.cartService.totalCartItemUpdated$.subscribe(
      (res) => {
  ...
  getProductSuggestions(query: string) {
    ...
    this.productService
      .getProducts({ q: query, select: 'title' }, { limit: 10, skip: 0 })
      .subscribe((res) => {
