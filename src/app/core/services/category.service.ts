...
export class CategoryService {
  ...
  getCategories(): Observable<Category[]> {
    ...
    return this.httpClient.get<string[]>(AppConstants.apiBaseUrl+'products/categories')
      .pipe(
        map((categories) => {
          const categoriesArr = categories.map((category, index) => {
            return {
              ...
              thumbnail: (category==='laptops') ? `https://cdn.dummyjson.com/product-images/${(index * 5) + 5}/thumbnail.jpeg`:
